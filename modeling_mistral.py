...
# Copied from transformers.models.llama.modeling_llama.LlamaLinearScalingRotaryEmbedding with Llama->Mistral
class MistralLinearScalingRotaryEmbedding(MistralRotaryEmbedding):
 ...
 #   def forward(self, x, seq_len=None):
 #       #added to resoleve bug... to be fixed directly in original forward
 #       # x: [bs, num_attention_heads, seq_len, head_size]
 #       if seq_len > self.max_seq_len_cached:
 #           self._set_cos_sin_cache(seq_len=seq_len, device=x.device, dtype=x.dtype)
 #       
 #       return (
 #           self.cos_cached[None, None, :seq_len, ...].to(dtype=x.dtype),
 #           self.sin_cached[None, None, :seq_len, ...].to(dtype=x.dtype),
 #       )


# Copied from transformers.models.llama.modeling_llama.LlamaDynamicNTKScalingRotaryEmbedding with Llama->Mistral
...
class MistralAttention(nn.Module):
    ...
    def __init__(self, config: MistralConfig):
        ...
        #)
        ####################################### Context Extension Code ##################################
        self._init_rope()
        #################################################################################################

    def _init_rope(self):
    # Added for Context Extension, from LLaMA implementation
...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            ################# Adrien add #################################
            shift_labels = shift_labels.to(torch.int64)
            ##############################################################
